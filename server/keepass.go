...
func searchUUIDs(clientGroup []gokeepasslib.Group, serverDb *gokeepasslib.Database, serverEntries map[gokeepasslib.UUID] gokeepasslib.Entry){
	...
	for _, clientElement := range clientGroup{
		...
		searchUUIDs(clientElement.Groups, serverDb, serverEntries)
	}
}

func compareDatabases(clientDb *gokeepasslib.Database, serverDb *gokeepasslib.Database){
	// right now we are only adding and changing entries
	serverEntries := getMapForAllEntries(serverDb)

	searchUUIDs(clientDb.Content.Root.Groups, serverDb, serverEntries)
}
